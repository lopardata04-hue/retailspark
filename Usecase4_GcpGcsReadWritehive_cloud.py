...
def main():
   ...
   gcs_df = spark.read.csv("gs://learning-hdfs-data/custs",mode='dropmalformed',schema=custstructtype1)
   gcs_df.show(10)
   ...
   gcs_df.repartition(2).write.json("gs://learning-hdfs-data/cust_output_json_"+curts)
   ...
   curts = spark.createDataFrame([1], IntegerType()).withColumn("curts", current_timestamp()).select(date_format(col("curts"), "yyyyMMddHHmmSS")).first()[0]
   print(curts)
   gcs_df.repartition(2).write.mode("overwrite").csv("gs://learning-hdfs-data/cust_csv"+curts)
   print("gcs Write Completed Successfully")
